...
export type AntigravityParticleShape =
  | "capsule"
  | "sphere"
  | "box"
  | "tetrahedron";
...
export interface AntigravityProps {
  ...
  depthFactor?: number;
  pulseSpeed?: number;
  particleShape?: AntigravityParticleShape;
...
function AntigravityInner({
  ...
  useEffect(() => {
    ...
    const onTouch = (e: TouchEvent) => {
      ...
      const t = e.touches[0];
      const rect = canvas.getBoundingClientRect();
      pointer.current.x = ((t.clientX - rect.left) / rect.width) * 2 - 1;
      pointer.current.y = -(((t.clientY - rect.top) / rect.height) * 2 - 1);
    };
